...
function App() {
  ...
  const handleFetchJira = async () => {
    ...
    try {
      const jiraIssue = await fetchJiraIssue(jiraId)
      setFormData({
        storyTitle: jiraIssue.summary,
        description: jiraIssue.description || '',
        acceptanceCriteria: jiraIssue.acceptanceCriteria || '',
        additionalInfo: `JIRA Status: ${jiraIssue.status}\nJIRA ID: ${jiraIssue.key}`
      })
    } catch (err) {
